...
function kikThis() {
    Clay.Kik.post( { message: 'I just scored ' + score + ' in Flappy Words! Think you can beat my score?', title: 'Flappy Words!' } );
}
...
function start() {
    ...
    // Show definition
    pickCorrectWord();
    scoreText.setText(correctDef);//DEFINITION HERE
...
function setGameOver(daBird, daTower) {

    if(daTower){
        if(daTower.word == correctWord){
            /*WIN GAME*/
            alert("YOU WIN!");
        }
    }
...
function update() {
    if (gameStarted) {
        ...
        towers.forEachAlive(function(tower) {
            /*TANNER WAS HERE*/
            tower.vocabText.x = tower.x;

            if (tower.x + tower.width < game.world.bounds.left) {
                tower.kill();
                tower.vocabText.setText("");
            }
        });
        // Update tower timer
        towersTimer.update();
